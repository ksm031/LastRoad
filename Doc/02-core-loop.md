...
graph TD
    ...
    J --> K
    K --> L{거점 체류 시간 판단}
    L -->|신속 처리| M[파츠 교체 & 수리 & 경로 선택]
    L -->|오래 머묾| N[괴물 간격 위험 수준 근접]
    M --> O[다음 구간 진입]
    N --> O
    O --> A
```
...
2. **위협과 마주친다** — 자연 위협(파츠 손상) / 컬트(정신력 손상) / 갓길 자원(멈추면 괴물이 가까워짐)
3. **거점에 도달한다** — 수리, 파츠 교체, 경로 선택. 체류 시간이 길수록 괴물이 가까워진다
4. **다음 구간으로 간다** — 구간이 깊어질수록 컬트 밀도 증가, 괴물 간격 누적 압박
